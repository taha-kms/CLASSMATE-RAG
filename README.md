...
By default nothing leaves your machine. Documents are parsed, embedded and
indexed locally, and answers are generated by a local model.
...
The application says so the first time a hosted provider is used in a
session, and every answer reports which backend produced it:
